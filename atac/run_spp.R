...
if (! is.na(iparams$chrname.rm.pattern)) {
  #	modified by kjyi
  rm.pattern <- strsplit(iparams$chrname.rm.pattern, ",")[[1]]
  name <- names(chip.data$tags)
  selectidx <- which(unlist(lapply(name, function(n) all(unlist(lapply(rm.pattern, function(r) !grepl(r, n)))))))
